...
class bag3_sync_sar_adc__strongarm_core(Module):
    """Module for library bag3_sync_sar_adc cell strongarm_core.
    ...
    def design(self, lch: int, seg_dict: Mapping[str, int], w_dict: Mapping[str, int],
        ...
        for name in ['os', 'in', 'tail', 'nfb', 'pfb', 'swo', 'swm']:
            if name=='os':
                self.remove_instance('XOSP') #TODO: fix layout so OSP can be included
                self.remove_instance('XOSN')
                self.remove_pin('osp')
                self.remove_pin('osn')
                continue
            uname = name.upper()
            ...
            if name == 'tail':
                inst_name = 'XTAIL'
                self.instances[inst_name].design(l=lch, w=w, nf=nf, intent=intent)
            else:
        ...
        if has_bridge:
            ...
            seg = seg_dict['br']
            intent = th_dict['br']
            self.instances['XBRO'].design(l=lch, w=w, nf=nf, intent=intent)
            self.instances['XBRM'].design(l=lch, w=w, nf=nf, intent=intent)
            if stack_br == 1:
                self.reconnect_instance_terminal('XBRO', 'g', 'clk')
                self.reconnect_instance_terminal('XBRM', 'g', 'clk')
            else:
                self.reconnect_instance_terminal('XBRO', f'g<{stack_br - 1}:0>', 'clk')
