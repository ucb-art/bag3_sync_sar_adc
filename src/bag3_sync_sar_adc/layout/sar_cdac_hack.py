...
class CapDrvArray(MOSBase):
    "Makes an array of switches, useful for custom spacing"
    ...
    def get_params_info(cls) -> Dict[str, str]:
        return dict(
            ...
            sp='dummy seperation',
            w='widths.',
            dum_row_idx_list='Index of dummy rows',
            sw_type='Type of switch',
            nx='number of columns',
            ny_list='number of rows',
            row_idx_list='list of row locations by tile index'
        )
    ...
    def draw_layout(self):
        ...
        ny_list: List[int] = self.params['ny_list']
        row_list: List[int] = self.params['row_idx_list']
        ...
        pinfo0 = [TilePatternElement(pinfo[1]['drv_tile'])] * (row_list[-1]+ny_list[-1])
        self.draw_base((TilePattern(pinfo0), pinfo[1]))

        dum_row_idx_list: List[List[int]] = self.params['dum_row_idx_list']
        ...
        idx = 0
        for ny, row, dum_row_idx in zip(ny_list, row_list, dum_row_idx_list):
            for i in range(ny):
                if idx > 0:
                    if dum_row_idx and i in dum_row_idx:
                        continue
                    ...
                    for jdx in range(nx):
                        ...
                        # idx = -(-(i+row)//rows_p_tile)
                        # print("TILE rows ", self.num_tile_rows)
                        # print(idx)
                        sw_col += seg + sp
...
class CapDacColCore(TemplateBase):
    ...
    def draw_layout(self) -> None:
        ...
        if (cap_config['ismim']):
            if sw_type.count('n') == 3:
                ...
                for idx, (ny, row, bit) in enumerate(zip(ny_list, row_list, bit_list)):
                    ...
                    unit_drv = 1634 #sw_n_master.bound_box.yh
                    print("Unit Drv: ",unit_drv, (sw_n_master.bound_box.yh), int(cap_master.array_box.yh) )
                    print(sw_y)
                    sw_right_coord = sw_n_master.bound_box.xh + sw_p_master.bound_box.xh + sw_x
                    row_idx = self.grid.coord_to_track(hm_layer, sw_y, mode=RoundMode.NEAREST, even=False)
                    sw_y = int(sw_y + max( (sw_n_master.bound_box.yh), unit_drv,
                                    int(cap_master.array_box.yh) ))
                    sw_y = -(-sw_y//h_blk)*h_blk 
                    capmim_y.append(sw_y)
                    
                    row_idx_list.append(int(round(row_idx)))
                    dum_row_idx_list.append([sum(ny_list[:nbits - diff_idx + 1]) + 1])
                    
                
                print('ROW_NUM: ',row_idx_list)
                print('DUM_ROW: ',dum_row_idx_list)
        ...
        if (cap_config['ismim'] == True):
            sw_n_arr_params = dict(
                    cls_name=CapDrvArray.get_qualified_name(),
                    draw_taps=True,
                    params=dict(pinfo=self.params['pinfo'], seg=seg, ny_list=ny_list, w=w_n, sp=sp, nx=2, sw_type='nch',
                                dum_row_idx_list=dum_row_idx_list, min_height=unit_cap_height, row_idx_list=row_idx_list)
                    )
            sw_p_arr_params = dict(
                    cls_name=CapDrvArray.get_qualified_name(),
                    draw_taps=True,
                    params=dict(pinfo=self.params['pinfo'], seg=seg, ny_list=ny_list, w=w_p, sp=sp, nx=1, sw_type='pch',
                            dum_row_idx_list=dum_row_idx_list, min_height=unit_cap_height, row_idx_list=row_idx_list)
                    )
            
            sw_n_arr_master = self.new_template(GenericWrapper, params=sw_n_arr_params)
            sw_p_arr_master = self.new_template(GenericWrapper, params=sw_p_arr_params)

            start_h = -(-capmim_y[0] // h_blk) * h_blk
            sw_p = self.add_instance(sw_p_arr_master, inst_name='XSWP', xform=Transform(sw_x, start_h))
            sw_n = self.add_instance(sw_n_arr_master, inst_name='XSWN', xform=Transform(swn_x, start_h))
            sw_ctrl_m = sw_n.get_all_port_pins('ctrl0')
            sw_ctrl_n = sw_n.get_all_port_pins('ctrl1')
            sw_ctrl_p = sw_p.get_all_port_pins('ctrl0')
            print("SW: ", 0)
            ...
            self.reexport(sw_p.get_port('VDD'), connect=True)
            self.reexport(sw_n.get_port('VSS'), connect=True)
            vdd_vm = sw_p.get_all_port_pins('VDD')
            vss_vm = sw_n.get_all_port_pins('VSS')
            vdd_xm = sw_p.get_all_port_pins('VDD_xm')
            vss_xm = sw_n.get_all_port_pins('VSS_xm')
